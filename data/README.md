You are in `/data`. The scripts in here mainly exist to create a `TFRecord` and associated manifest csv. The tensorflow dataset contains image tensors for a subsample of the raw data, along with metadata.

The public repository contains three Python scripts and a json file, though the scripts  require and assume locally accessible data with this directory structure:
...
- `load_tfrecord.py` defines an exportable function that reads those records and returns a `tf.data.Dataset`. For specific workflows the loading logic can be modified, this just allows for quickly setting up, eg. in a notebook.

If you have the `cases.tfrecord` in `/data`, you do not need the raw JPGs or CSVs to load it. 

ie. : 
```python
from data.load_tfrecord import load_tfrecord
