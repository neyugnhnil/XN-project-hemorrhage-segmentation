...
def build_model_from_config(cfg: dict):
    return build_cnn(
        ...
        conv_filters=tuple(cfg["cnn"]["conv_filters"]),
        kernel_size=int(cfg["cnn"]["kernel_size"]),
        dense_units=tuple(cfg["cnn"]["dense_units"]),
        dropout_rate=float(cfg["cnn"]["dropout_rate"]),
        use_batch_norm=bool(cfg["cnn"]["use_batch_norm"]),
        global_pool=str(cfg["cnn"]["global_pool"])
        )
...
def main() -> None:
    ...
    model = compile_classification_model(
        model,
